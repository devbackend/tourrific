...
<body>
    ...
    <?php
        if(!Auth::check())
            echo '<form action="/public/login" method="POST"><input type="text" name="login" value="" /><input type="password" name="pass" value="" /><input type="submit" name="doLOgin" value="Log in" /></form>';
        else
            echo '<h2>'.Auth::user()->login.'</h2>';
    ?>
